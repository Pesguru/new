...
module.exports = {
  ...
  operate: async (context) => {
    const { q: text, CzAx, m, prefix, command, zreply, reaction, sleep, crtImg } = context;

    if (!text) {
      await zreply(`Add input, Example: *${prefix + command} nature*`);
    ...
    try {
      ...
      // End - Send Carousel
      await CzAx.relayMessage(m.chat, {
        viewOnceMessage: {
          message: {
            interactiveMessage: {
              body: {
      ...
      await zreply('Failed to fetch Pinterest images. Please try again later.');
      await reaction(m.chat, "❌");
    }
  }
};