...
module.exports = {
    ...
    operate: async (context) => {
        ...
        if (!q) {
            await zreply(`Add input, Example: *${prefix + command} Hello World*`);
        ...
        if (command === 'qrcode') {
            try {
                ...
                await CzAx.sendMessage(m.chat, { image: fs.readFileSync(outputFile) }, { quoted: m });
                await reaction(m.chat, "✅");
                fs.unlinkSync(outputFile);
            } catch (err) {
                console.error('Error generating QR code:', err);
                await zreply('Failed to generate QR code. Please try again later.');
                await reaction(m.chat, "❌");
            }
            ...
            try {
                ...
                await zreply('Failed to generate barcode. Please try again later.');
                await reaction(m.chat, "❌");
            }
        }
    }
